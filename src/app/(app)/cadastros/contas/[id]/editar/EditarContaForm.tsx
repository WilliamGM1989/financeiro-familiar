...
export default function EditarContaForm({ account }: { account: AccountRow }) {
  ...
  return (
    <form onSubmit={handleSubmit} className="flex flex-col gap-4 px-6 py-4 pb-28">
      ...
      <div>
        ...
        <select id="type" name="type" required defaultValue={account.type} className={inputCls}>
          {accountTypes.map((t) => (
      ...
      {/* Saldo inicial */}
      <div>
        <label htmlFor="initial_balance" className={labelCls}>Saldo inicial (R$)</label>
        <input
          id="initial_balance"
